    ...
    */
	...
	func canonicalHeaders()->[(String, String)] {
		...
		headerValues = headerValues.filter({ (key0, _) -> Bool in
			return key0.lowercased() == "host"
				|| key0.lowercased() == "content-type"
				|| key0.lowercased().hasPrefix("x-amz-")
		})
    ...
    mutating func canonicalRequest()->(request:String, signedHeaders:String)? {
        guard let (beforePayload, signedHeaders) = canonicalRequestBeforePayload(urlQueryParams: nil) else { return nil }
        let hashedBody:String = sha256HashedBody.map { CryptoUtils.hexString(from: $0).lowercased() } ?? ""
	...
	mutating func hashCanonicalRequest(signPayload:Bool, urlQueryParams: Dictionary<String, String>? = nil) -> (hashedRequest:String, signedHeaders:String)? {
        guard let (request, signedHeaders) = canonicalRequest() else { return nil }
